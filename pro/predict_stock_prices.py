...
dataset_train = pd.read_csv('Netflix_Stock_Price_Train.csv')
...
dataset_test = pd.read_csv('Netflix_Stock_Price_Test.csv')
...
X_test = []
for i in range(INPUT_SIZE, INPUT_SIZE+len(dataset_test)):
...
plt.plot(predicted_stock_price, color = 'blue', label = 'Pred Train + Test')
...
plt.figure(1, figsize=(12, 5))
plt.plot(real_stock_price_all[-len(dataset_test):], color = 'red', label = 'Real')
plt.plot(predicted_stock_price[-len(dataset_test):], color = 'green', label = 'Pred Test Only')
plt.title('Netflix Stock Price Prediction')
plt.xlabel('Time')
plt.ylabel('Netflix Stock Price')
plt.legend()
